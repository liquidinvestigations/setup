...
if [ ! -e $SECRETS_PATH ]; then
  ...
  CLIENT_OAUTH_ID=$(sudo -u liquid-apps ./libexec/run-h bin/hypothesis authclient add --type public --name liquid --authority hypothesis.{{ liquid_domain }} --redirect-uri {{ http_scheme }}://hypothesis.{{ liquid_domain }}/app.html | tail -1 | cut -d ' ' -f 3)
